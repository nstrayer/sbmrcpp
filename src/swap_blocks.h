...
void swap_block(Node* child_node, Node* new_block, Node_Container& blocks, const bool remove_empty = true)
{
  ...
  if (remove_empty & (old_block->num_children() == 0)) {
    ...
    const bool delete_successful = delete_from_vector(blocks_of_type, old_block);
    if(!delete_successful) stop("Tried to delete a node that doesn't exist");
  }
